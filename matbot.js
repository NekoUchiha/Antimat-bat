...
robot.on('guildMemberAdd', member => {
 ...
 robot.on('guildMemberRemove', member => {
  ...
  var textBaybay = [
    ...
    `**${member.user.tag}** не нашёл Себя у нас и решил продолжить поиск в другом месте`,
...
if (GuildName === "EC") return msg.channel.send(`Дорогие Девочки Администрация Сервера(Дискорд) **${msg.guild.name}** поздравляет вас с 8 Марта `, {
		files: [
		  "./img/8mathecl.png"
		]
	  });
if (GuildName === "Ce9M") return msg.channel.send(`Дорогие Друзья Администрация Сервера(Дискорд) **${msg.guild.name}** поздравляет вас хоть и с прошедшим но 9 Мая :D `, {
		files: [
		  "./img/9maya2.png"
		]
	  });
if (GuildName === "Nogod") return msg.channel.send(`Дорогие Друзья Администрация Сервера(Дискорд) **${msg.guild.name}** поздравляет вас хоть и с Новым Годом и Желает всего самого наилучшего в этому Году `, {
		files: [
		  "./img/kzng.png"
		]
	  });
    ...
    if (newMessage.author.bot) return undefined;
     ...
     if( swearWords.some(word => newMessage.content.toLowerCase().includes(word)) ) {
		...
		if(NoBadWords.some(word => newMessage.content.toLowerCase().includes(word)) ) return;
		            newMessage.delete();
            msg.channel.send('**' + newMessage.author.username + '** - Ай яй яй Нельзя Говорить Такие Слова')
